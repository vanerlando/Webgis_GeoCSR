...
const DATA_URL = DATA_MODE === 'point'
    ? 'data/point_ses_jabar_final.geojson'
...
const state = {
    ...
    rankingMode: 'global',
    weights: { need: 34, proximity: 33, branding: 33 },
    displayLimit: 5,
    unselectedOpacity: 1,
    villages: null,
    filteredVillages: [],
...
function calculateMCDA() {
    const wSum = state.weights.need + state.weights.proximity + state.weights.branding;
    if (!wSum) return;
    const wN = state.weights.need / wSum, wD = state.weights.proximity / wSum, wB = state.weights.branding / wSum;
    ...
    state.filteredVillages.forEach(v => {
        ...
        // Penyesuaian Normalisasi Jarak: jika isochrone, asumsikan kecepatan rata-rata 30-40km/jam sebagai proxy jarak
        const maxDist = state.rangeMode === 'radius' ? state.radius : state.isochroneMin * 0.6; 
        p.normDist = Math.max(0, 1 - (minDist / maxDist));

        const skorNeed = p.skor_need || 0, skorBranding = p.skor_branding || 0;
        p.score = (skorNeed * wN) + (p.normDist * wD) + (skorBranding * wB);

        const contributions = [
            { name: 'Kebutuhan Sosial Tinggi', val: skorNeed * wN },
...
function renderCards() {
    ...
    topVillages.forEach((v, index) => {
        const p = v.properties;
        const scorePct = (p.score * 100).toFixed(1), needPct = ((p.skor_need || 0) * 100).toFixed(0), proximPct = ((p.normDist || 0) * 100).toFixed(0), brandPct = ((p.skor_branding || 0) * 100).toFixed(0), kpdPct = ((p.skor_kepadatan || 0) * 100).toFixed(0);
...
function exportCSV() {
    ...
    const headers = ['Rank','Desa','Kecamatan','Kabupaten','Pabrik Terdekat','Overlap 2+ Pabrik','Need (0-100)','Proximity (0-100)','Branding (0-100)','Total MCDA','SES','Jarak (km)'];
    ...
    const _doExport = () => {
        ...
        const wsData = [
            ['Rank','Desa','Kecamatan','Kabupaten','Pabrik Terdekat','Overlap 2+ Pabrik',
             'Need (0-100)','Proximity (0-100)','Branding (0-100)','Total MCDA','SES','Jarak (km)']
        ];
        data.forEach((v, i) => {
            const p = v.properties;
            wsData.push([
...
function updateDynamicLegend() {
    ...
    if (state.weights.need >= 50) modeName = "Kebutuhan";
...
['need', 'proximity', 'branding'].forEach(key => {
...
document.querySelectorAll('.preset-btn[data-preset]').forEach(btn => {
    btn.addEventListener('click', e => {
        document.querySelectorAll('.preset-btn').forEach(b => b.classList.remove('active')); e.target.classList.add('active');
        const mode = e.target.dataset.preset;
        const w = mode === 'need' ? { need: 60, proximity: 20, branding: 20 } : mode === 'proximity' ? { need: 20, proximity: 60, branding: 20 } : { need: 34, proximity: 33, branding: 33 };
        Object.keys(w).forEach(k => { state.weights[k] = w[k]; const s = document.getElementById(`slider-${k}`); const v = document.getElementById(`val-${k}`); if (s) s.value = w[k]; if (v) v.innerText = `${w[k]}%`; });
        runAnalysis();
    });
});
